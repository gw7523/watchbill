...
- [x] Name is Watchbill. No Muster collision. — **PASS** — unchanged; plugin id `agents.watchbill`; `upgrade-mux` is an action alias, not a rename.
...
## Checklist
- Product / name: PASS — CLI `watchbill`, plugin `agents.watchbill`, mux names herdr/tmux/cmux only (`src/watchbill/hosts.py:35`).
