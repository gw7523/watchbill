...
- [x] Name is Watchbill. No Muster collision. — **PASS** — product/CLI/plugin id `agents.watchbill`; tests reject `herdr-muster` / `kichel.muster`.
...
## Checklist
- Product / name: PASS — CLI `watchbill`, plugin `agents.watchbill`, no Muster/Fleet/Corral rename; `tests/test_skill_plugin.py:35` bans herdr-muster.
